...
def get_full_dataset(path_files, var, origin):
    ...
    # years = np.array([dt.datetime.strptime(list_files[i].split('_')[-1],'%Y-%m-%d.nc').year\
    #     for i in range(len(list_files))]) ## ERA5
    years = np.array([dt.datetime.strptime(list_files[i].split('.')[0][-10:].replace('_','-'),'%Y-%m-%d').year\
        for i in range(len(list_files))]) ## SODA
    
    for year in range(1980,2022):
        print(year)
        where_year = np.where(years == year)[0]
        dataset = xr.open_mfdataset(list_files[where_year],combine='nested',concat_dim="time")
        
        if dataset.time.dtype==np.int64:
            dataset['time'] = np.array([dt.datetime.strptime(list_files[where_year][i].split('_')[-1],\
            '%Y-%m-%d.nc')\
            for i in range(len(list_files[where_year]))])
        ...
        if origin == 'ERA5':
            dataset = dataset.assign_coords(lat=dataset.lat.values[:,0],lon=dataset.lon.values[0,:])
            dataset = dataset.rename({'x': 'lon', 'y': 'lat'})
        dataset.to_netcdf(f'/glade/scratch/jhayron/Data4Predictability/DailyDatasets/{var}/{var}_{str(year)}.nc')
